...
QtMgx3DGroupNamePanel::QtMgx3DGroupNamePanel (
	...
	int dimension, QtMgx3DGroupNamePanel::POLICY policy,
	const string& defaultName)
	: QWidget (parent), ValidatedField ( ),
	  _nameComboBox (0), _dimension (dimension), _mainWindow (&mainWindow),
	  _policy (QtMgx3DGroupNamePanel::INITIALIZATION),
...
QtMgx3DGroupNamePanel::QtMgx3DGroupNamePanel (const QtMgx3DGroupNamePanel&)
	: QWidget (0), ValidatedField ( ),
	  _nameComboBox (0), _dimension (-1), _mainWindow (0),
	  _policy (QtMgx3DGroupNamePanel::INITIALIZATION), _defaultName ( )
{
	MGX_FORBIDDEN ("QtMgx3DGroupNamePanel copy constructor is not allowed.");
}	// QtMgx3DGroupNamePanel::QtMgx3DGroupNamePanel (const QtMgx3DGroupNamePanel&)


QtMgx3DGroupNamePanel& QtMgx3DGroupNamePanel::operator = (
												const QtMgx3DGroupNamePanel&)
{
...
void QtMgx3DGroupNamePanel::setPolicy (
				QtMgx3DGroupNamePanel::POLICY policy, const string& defaultName)
{
	...
	switch (policy)
	{
		...
		case QtMgx3DGroupNamePanel::CONSULTATION		:
		{
			...
			for (vector<string>::iterator it = groupNames.begin ( );
			     groupNames.end ( ) != it; it++)
				_nameComboBox->addItem ((*it).c_str ( ));
			if ((false == defaultName.empty ( )) &&
			    (-1 != _nameComboBox->findText (defaultName.c_str ( ))))
				_nameComboBox->addItem (defaultName.c_str ( ));
			_nameComboBox->setEditable (QtMgx3DGroupNamePanel::CREATION ==  policy);
			if (false == defaultName.empty ())
				_nameComboBox->setCurrentIndex (
							_nameComboBox->findText (defaultName.c_str ( )));
		}	// case QtMgx3DGroupNamePanel::CREATION
		break;
		default	:
...
/*			if (getGroupName ( ) != getDefaultName ( ))
		...
		{
			UTF8String	error (Charset::UTF_8);
			error << "QtMgx3DGroupNamePanel::validate. Politique inconnue ("
			      << (unsigned long)getPolicy ( ) << ")";
			throw Exception (error);
		}
	}	// switch (getPolicy ( ))
...
void QtMgx3DGroupNamePanel::getGroupNames (vector< string >& names)
{
	...
	GroupManagerIfc&	groupManager	=
							_mainWindow->getContext ( ).getGroupManager ( );
	...
	switch (getDimension ( ))
	{
		case 0	:
		{
			...
			for (vector<Group0D*>::const_iterator it = groups.begin ( );
			     groups.end ( ) != it; it++)
		...
		case 1	:
		{
			...
			for (vector<Group1D*>::const_iterator it = groups.begin ( );
			     groups.end ( ) != it; it++)
				names.push_back ((*it)->getName ( ));
		}
		break;
		case 2	:
		{
			...
			for (vector<Group2D*>::const_iterator it = groups.begin ( );
			     groups.end ( ) != it; it++)
				names.push_back ((*it)->getName ( ));
		}
		break;
		case 3	:
		{
			...
			for (vector<Group3D*>::const_iterator it = groups.begin ( );
			     groups.end ( ) != it; it++)
				names.push_back ((*it)->getName ( ));
		}
		break;
		...
		{
			UTF8String	error (Charset::UTF_8);
			error << "QtMgx3DGroupNamePanel::getGroupNames : "
			      << "dimension invalide (" << (long)getDimension ( ) << ").";
			throw Exception (error);
		}
	}	// switch (getDimension ( ))
