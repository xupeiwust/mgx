...
#include <DKOC/DKOC_GetShapeInfo.hpp>
//#include <DKOC/cv5occ_acces_protec.h>
...
void ImportCATIAImplementation::readFile()
{
    ...
    //int stat = cv5occ_topods_data((char*)(m_filename.c_str()), data, -1, -1);
    int CatalogItem = 0;
    int stat = OCCTopoDS_Read_Data ((char*)(m_filename.c_str()),data,CatalogItem);

    std::cout<<"Importation du fichier Catia : "<<m_filename<<std::endl;
    std::cout<<"retour fonction importation = "<<stat<<std::endl;
    std::cout<<"CATIA Nb parts       = "<<data.NbParts()<<std::endl;
    std::cout<<"CATIA Nb faces       = "<<data.NbFaces()<<std::endl;
    std::cout<<"CATIA Nb free curves = "<<data.NbFreeCurves()<<std::endl;
    std::cout<<"CATIA Nb free points = "<<data.NbFreePoints()<<std::endl;
    ...
    m_importedShapes[0]= data.RootShape();
#else
