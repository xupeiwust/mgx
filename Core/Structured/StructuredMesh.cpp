...
StructuredMesh::StructuredMesh (const string& type, int ni, int nj, int nk, bool allocate)
	  ...
	  _densities (0), _densityMin (0.), _densityMax (0.)
{
	if (true == allocate)
	{
		_abs	= new float [_ni + 1];
		memset (_abs, 0, (_ni + 1) * sizeof (float));
		_ord	= new float [_nj + 1];
		memset (_ord, 0, (_nj + 1) * sizeof (float));
		_elev	= new float [_nk + 1];
		memset (_elev, 0, (_nk + 1) * sizeof (float));
	}	// if (true == allocate)
...
void StructuredMesh::setPureCellNum (int num)
{
	if (num < 0)
	{
		UTF8String	error (Charset::UTF_8);
		error << "Affectation au maillage de type " << type ( ) << " d'un nombre négatif de mailles pures.";
		throw Exception (error);
	}	// if (num < 0)

	_pureCellNum	= num;
	_mixedCellNum	= _cellNum - num;
	_pureIds	= new int [_pureCellNum];
	memset (_pureIds, 0, _pureCellNum * sizeof (int));
	_mixedIds	= new int [_mixedCellNum];
	memset (_mixedIds, 0, _mixedCellNum * sizeof (int));
}	// StructuredMesh::setPureCellNum


void StructuredMesh::setDensityRange (float min, float max)
{
	if ((min > max) || (min < 0.))
	{
...
void StructuredMesh::setCellDensities (float* densities)
{
